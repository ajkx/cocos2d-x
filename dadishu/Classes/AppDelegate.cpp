...
bool AppDelegate::applicationDidFinishLaunching() {
	...
	if(CC_TARGET_PLATFORM == CC_PLATFORM_IOS){
		if(frameSize.width >960){
			director->setContentScaleFactor(MIN(640/640,1136/960));
		}

	}else if(CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID){
        Size resourceSize = Size(480, 800);
        // 1280X720安卓传说中的HD，也是现在各品牌主流旗舰机型的分辨率，而且屏幕都很大
        if(frameSize.width == 1280 && frameSize.height == 720){
            resourceSize = Size(1280, 720);
        }
        director->setContentScaleFactor(MIN(resourceSize.height/640,
                                            resourceSize.width/960));
	}
